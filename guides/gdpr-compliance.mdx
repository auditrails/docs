...
**Deadline evidence:** GDPR Art.12(3) requires a response without undue delay and in any event within one month. The DSAR workflow logs a `data.deleted` event with a timestamp so you can demonstrate when the request was actioned.

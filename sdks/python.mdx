...
`log()` never raises. For `log_direct()` and `log_batch_direct()`, catch `AuditRailsError` to inspect the failure details.

```python
from auditrails import AuditRailsError, AuditEvent

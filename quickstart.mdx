...
<Steps>
  ...
  <Step title="Install the SDK (or skip it)">
    <Note>
      **Java is not on Maven Central yet** — build it from source or use the `curl` tab. The other four install normally. Full status on the [SDK overview](/sdks/overview).
    ...
    ```bash Java
    # Not on Maven Central yet. Build from source, or call

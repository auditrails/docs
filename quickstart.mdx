...
<Steps>
  ...
  <Step title="Install the SDK (or skip it)">
    <Note>
      **Node.js and Go are on their registries.** Python, PHP and Java are on GitHub tagged `v1.0.0` but not yet on PyPI, Packagist or Maven Central. Python and PHP install from Git in the meantime; Java needs the REST API for now. Full status on the [SDK overview](/sdks/overview).
    ...
    ```bash Python
    # PyPI package pending. Installs from Git today:
    pip install git+https://github.com/auditrails/auditrails-python.git
    ```

    ```bash PHP
    # Packagist package pending. Add the repository, then require it:
    composer config repositories.auditrails vcs https://github.com/auditrails/auditrails-php
    composer require auditrails/auditrails-php guzzlehttp/guzzle
    ```


    ```bash Java
    # Maven Central artifact pending. Build from source, or call
    # POST /v1/events directly until it lands.
    ```

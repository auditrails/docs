...
<Steps>
  ...
  <Step title="Install an SDK">
    Install the SDK for your language, or skip it entirely and call the REST API. `npm install @auditrails/node` and `go get github.com/auditrails/auditrails-go` are on their registries today; Python and PHP install from Git; Java is source-only for now. See the [SDK overview](/sdks/overview) for current status per language.
  </Step>

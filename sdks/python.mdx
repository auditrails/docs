...
<Note>
  **Not on PyPI yet**, so `pip install auditrails` does not resolve. Install
  from Git in the meantime. Everything else on this page works unchanged.
</Note>
...
<Tabs>
  ...
  <Tab title="Async (httpx)">
    ```bash
    pip install "auditrails[async] @ git+https://github.com/auditrails/auditrails-python.git"
    ```
...
Once it is on PyPI these become `pip install auditrails` and `pip install auditrails[async]`.

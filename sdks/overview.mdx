...
<Note>
  **Node.js and Go are on their public registries.** Python, PHP and Java are published as source on GitHub, tagged `v1.0.0`, but are not yet on PyPI, Packagist or Maven Central, so `pip install auditrails` does not resolve yet.

  Python and PHP install directly from Git in the meantime, and every SDK is a thin wrapper over a plain REST API you can call with `curl` today. See the table below for what works right now per language.
</Note>
...
<CardGroup cols={2}>
  <Card title="Node.js / TypeScript" icon="node-js" href="/sdks/node">
    **On npm.** Requires Node 18+ for native `fetch`. Zero runtime dependencies.
  </Card>
  <Card title="Go" icon="golang" href="/sdks/go">
    **On the Go proxy.** Uses only the standard `net/http` package. Zero dependencies.
  </Card>
  <Card title="Python" icon="python" href="/sdks/python">
    **Install from Git.** Sync client uses `urllib` (zero deps); async client adds `httpx`.
  </Card>
  <Card title="PHP" icon="php" href="/sdks/php">
    **Install from Git.** PSR-18 compatible, works with Guzzle or any PSR-18 HTTP client.
  </Card>
...
| Language | Registry status | How to install today | Repository |
|---|---|---|---|
| Node.js / TypeScript | **On npm** | `npm install @auditrails/node` | [auditrails-node](https://github.com/auditrails/auditrails-node) |
| Go | **On the Go proxy** | `go get github.com/auditrails/auditrails-go` | [auditrails-go](https://github.com/auditrails/auditrails-go) |
| Python | Pending on PyPI | `pip install git+https://github.com/auditrails/auditrails-python.git` | [auditrails-python](https://github.com/auditrails/auditrails-python) |
| PHP | Pending on Packagist | Composer VCS entry, see the [PHP page](/sdks/php) | [auditrails-php](https://github.com/auditrails/auditrails-php) |
| Java | Pending on Maven Central | Build from source, see the [Java page](/sdks/java). Use the REST API. | [auditrails-java](https://github.com/auditrails/auditrails-java) |

Every SDK is a thin client over the same REST endpoint. If your language is not ready yet, or you would rather not take a dependency, [`POST /v1/events`](/api-reference/events/create) is the whole interface and is fully supported.

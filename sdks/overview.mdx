...
description: "Official AuditRails SDKs for Node.js, Python, Go, and PHP. Automatic batching, exponential-backoff retries, and graceful shutdown built in."
---
...
<Note>
  Every SDK is a thin client over one REST endpoint. If your language is not
  listed, [`POST /v1/events`](/api-reference/post-events) is the whole
  interface and is fully supported.
</Note>
...
If you would rather not take a dependency at all, [`POST /v1/events`](/api-reference/post-events) is the whole interface and is fully supported.
...
| **Graceful shutdown** | Each SDK registers a process-exit hook (`beforeExit`/`SIGTERM` in Node, `atexit` in Python, `defer client.Close()` in Go, `register_shutdown_function` in PHP) to flush any remaining buffered events before the process terminates. |
| **Minimal dependencies** | Node uses native `fetch`, Python uses `urllib`, Go uses `net/http`, and PHP requires a PSR-18 HTTP client (e.g. Guzzle). |
